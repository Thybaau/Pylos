...
import {
  Landmark, Plus, Pencil, Trash2, X, Check, RotateCcw, AlertTriangle, CheckCircle, XCircle,
} from 'lucide-react'

interface OrgFormState { name: string; description: string; is_active: boolean }
const DEFAULT_FORM: OrgFormState = { name: '', description: '', is_active: true }
...
export default function Organizations() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['organizations'], queryFn: organizationsApi.getAll })
  ...
  return (
    <div className="p-6 space-y-6 overflow-y-auto h-full">
      ...
      <div className="rounded-xl border border-zinc-800/50 bg-zinc-900/30 overflow-hidden">
        <table className="w-full text-sm">
          <thead className="border-b border-zinc-800/50">
            <tr>{['Name', 'Description', 'Status', ''].map(h => <th key={h} className="text-left px-5 py-3.5 text-xs text-zinc-500 uppercase tracking-wide font-medium">{h}</th>)}</tr>
          </thead>
          <tbody>
            {isLoading ? Array.from({ length: 3 }).map((_, i) => (
              <tr key={i} className="border-b border-zinc-800/30">{Array.from({ length: 4 }).map((_, j) => <td key={j} className="px-5 py-3.5"><div className="h-3 bg-zinc-800 rounded animate-pulse w-24" /></td>)}</tr>
      ...
      {editing && <OrgModal initial={{ name: editing.name, description: editing.description || '', is_active: editing.is_active }} isEdit={true} onClose={() => { setEditing(null); setMutationError(null) }} onSave={f => updateMut.mutate({ id: editing.id, f })} isSaving={updateMut.isPending} error={mutationError} />}
